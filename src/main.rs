...
fn main() {
    ...
    if args.len() != 4 {
        println!("Bitte verwende {} <stree|vebtree|btree|binary> <u40|u48|u64> <uniform|normal|normal|bwt_runs>",args[0]);
        return;
    }
	
    if args[3] != "uniform" && args[3] != "normal" && args[3] != "bwt_runs" {
        println!("Bitte verwende {} <stree|vebtree|btree|binary> <u40|u48|u64> <uniform|normal|bwt_runs>",args[0]);
        return;
    } 
	...
	match args[2].as_ref() {
		...
		"u48" => stage1::<u48>(args),
		"u64" => stage1::<u64>(args),
		_ => println!("Bitte verwende {} <stree|vebtree|btree|binary> <u40|u48|u64> <uniform|normal|bwt_runs>",args[0]),
    }
}
